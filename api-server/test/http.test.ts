...
test("GET / is a friendly public root", async () => {
...
test("protected route without a token returns 401 (the 'Missing bearer token' case)", async () => {
  const res = await fetch(`${base}/auth/me`);
  assert.equal(res.status, 401);
  const body = (await res.json()) as { message: string };
  assert.match(body.message, /bearer token/i);
});
