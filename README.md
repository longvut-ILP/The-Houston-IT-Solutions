...
Nail Employee salary/      ← the API (deploy as Vercel project #2, root = this folder)
...
## Deploy (summary — full steps in `Nail Employee salary/DEPLOY.md`)

1. Push this repo to GitHub.
2. **Vercel project #1** = the website (root = repo root). The demo works immediately.
3. **Neon**: create a Vercel Postgres database, run `Nail Employee salary/db/neon-setup.sql`
   in its SQL Editor (set the owner password first).
4. **Vercel project #2** = the API (root = `Nail Employee salary`). Set env vars
